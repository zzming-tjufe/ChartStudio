...
TEMPLATE_GROUP_WHITELIST: Dict[str, Set[str]] = {
    ...
    "bar_chart_basic": {
        "基础信息", "画布尺寸", "字体设置", "坐标轴", "图例", "柱状图样式",
        "颜色设置", "数据标签", "导出设置",
    },
    "horizontal_bar_chart": {
        "基础信息", "画布尺寸", "字体设置", "坐标轴", "柱状图样式",
        "颜色设置", "数据标签", "导出设置",
    },
    "heatmap_basic": {
        "基础信息", "画布尺寸", "字体设置", "坐标轴", "热力图样式", "导出设置",
    },
    "scatter_chart_basic": {
        "基础信息", "画布尺寸", "字体设置", "坐标轴", "图例", "散点样式",
        "颜色设置", "导出设置",
    },
}
...
def render_simple_panel(
    ...
    for group in SIMPLE_MODE_GROUPS:
        if not _group_allowed(group, template_id):
            continue
        if not _group_has_fields(config, group):
            continue

        with st.expander(
            group["title"],
            expanded=group["title"] in ("基础信息", "画布尺寸", "字体设置"),
        ):
            if group.get("canvas_pair"):
                pw, ph = group["canvas_pair"]
                if not (path_exists(config, pw) and path_exists(config, ph)):
                    alt = group.get("canvas_pair_alt")
                    if alt:
                        pw, ph = alt
                if path_exists(config, pw) and path_exists(config, ph):
            ...
            if group.get("font_settings"):
                font_val = get_by_path(config, "font")
    ...
    if isinstance(data, dict) and data:
        with st.expander("数据（只读）", expanded=False):
            st.caption("数据请在「图表配置文件」中编辑，或替换 data/ 目录下的数据文件。")
